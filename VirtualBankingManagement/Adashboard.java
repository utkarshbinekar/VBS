    ...
    Adashboard() {
        ...
        b1.addActionListener(
                a->{
                    ...
                    double min=(str1.isEmpty()) ? 0.0 : Double.parseDouble(str1);
                    double max=(str2.isEmpty()) ? Double.MAX_VALUE : Double.parseDouble(str2);

                    String url ="jdbc:mysql://localhost:3306/3dec";
                    try(Connection corn = DriverManager.getConnection(url,"root","Utkarsh05")){
                        String u = "select * from users where balance between "+min+" and "+max;
                        try(PreparedStatement pst = corn.prepareStatement(u)){
